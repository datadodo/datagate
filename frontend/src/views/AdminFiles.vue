    ...
    <header class="glass-card m-4 rounded-xl">
      <div class="flex items-center justify-between p-6">
        ...
        <div class="flex items-center space-x-4">
          ...
          <div>
            <h1 class="text-2xl font-bold text-white">All Files</h1>
            <p class="text-white/70 text-sm">Manage all uploaded files</p>
          </div>
        </div>
          ...
          <div class="p-6">
            ...
            <div v-else-if="filteredFiles.length === 0" class="text-center py-12">
              ...
              <p class="text-white/70 mb-4">
                {{ hasActiveFilters ? 'Try adjusting your filters' : 'Files will appear here once users start uploading' }}
              </p>
              <button
                v-if="hasActiveFilters"
                @click="clearFilters"
            ...
            <!-- Files List -->
            <FilesManagement :files="filteredFiles" @fileDeleted="refreshFiles" v-else />
          </div>
        </div>
      </div>
    </main>
...
const filteredFiles = computed(() => {
  ...
  // Owner filter
  if (filters.owner) {
    result = result.filter(file => file.user_id === filters.owner)
  }
...
const hasActiveFilters = computed(() => {
  return filters.search || filters.owner || filters.fileType || filters.sizeRange || filters.dateRange
})

// Methods
const refreshFiles = async () => {
  try {
    await adminStore.fetchAllFiles()
  } catch (error) {
...
onMounted(async () => {
  try {
    ...
    console.error('Error loading admin files data:', error)
  }
})
</script>
